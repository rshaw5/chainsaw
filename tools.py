...
def create_summarize_tool(llm, memory):
    ...
    return LLMChain(llm=llm, prompt=summarize_prompt, verbose=True, memory=memory)

def create_refine_tool(llm, memory):
    ...
    return LLMChain(llm=llm, prompt=refine_prompt, verbose=True, memory=memory)
